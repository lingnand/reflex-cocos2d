...
space :: ( NodeBuilder t m, MonadIO m
         , MonadFinalize m, PerformEvent t m
         , MonadIO (Performable m), MonadIO (Finalizable m) )
...
space props = do
    ...
    fmap (wrapped,) . performEvent . ffor ts $ \dt -> liftIO $ do
...
getCollisionEvents ::
  ( NodeBuilder t m, MonadIO m, FastTriggerEvent t m)
  => Space a -> m (CollisionEvents t a)
getCollisionEvents (SPWrap sp) = do
    (eBegin, trBegin) <- fastNewTriggerEvent
    (eSeparate, trSeparate) <- fastNewTriggerEvent
            ...
            Just (sa, sb) -> do
              ...
              if saa^.H.shapeCollisionMask .&. sab^.H.shapeCategoryMask == 0
                ...
                then return False
                  ...
                  H.postStep sa . liftIO $ trBegin (toSA saa, toSA sab)
                  return True
            _ -> return False
      , H.preSolveHandler = Nothing
      , H.postSolveHandler = Nothing
      , H.separateHandler = Just $
          -- XXX: how to not check shapes if we don't even have the trigger?
          H.shapes >>= \case
            Just (sa, sb) -> do
              saa <- S.get (H.shapeAttributes sa)
              sab <- S.get (H.shapeAttributes sb)
              H.postStep sa $
                liftIO $ trSeparate (toSA saa, toSA sab)
            _ -> return ()
      }
...
body :: ( MonadIO m, MonadFinalize m
...
shape :: ( MonadIO m, MonadFinalize m
         , MonadIO (Finalizable m) )
      => Space a
      -> Body a
      -> H.Geometry Float
