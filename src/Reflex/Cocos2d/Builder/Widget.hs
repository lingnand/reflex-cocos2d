...
getWidgetTouchEvents w = do
    touchTypes <- fastNewEventWithLazyTriggerWithOnComplete $ \triggerFunc -> do
      widget_addTouchEventListener w $ \_ t -> triggerFunc t (return ())
      return $ pure ()
...
getWidgetClicks :: (WidgetPtr w , FastTriggerEvent t m) => w -> m (Event t ())
getWidgetClicks w = fastNewEventWithLazyTriggerWithOnComplete $ \triggerFunc -> do
